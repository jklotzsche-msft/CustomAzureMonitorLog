<#
    .SYNOPSIS
    Add-CAMLTeamsDeviceUsageData adds Teams device usage data to a custom log analytics workspace.
    ...
    Step 2:
    Get Teams Device Usage Reports as JSON object.
	Therefore, the cmdlet "Get-CAMLTeamsDeviceUsage" is used.
    ...
    .EXAMPLE
    Add-CAMLTeamsDeviceUsageData -VaultCertificateName cert-CustomAzureMonitorLog -VaultName kv-CustomAzureMonitorLog -ClientId 00000000-0000-0000-0000-000000000000 -TenantId 00000000-0000-0000-0000-000000000000 -WorkspaceId 00000000-0000-0000-0000-000000000000 -LawTable CAMLTeamsDeviceUsage_CL -DceLogsIngestionUri https://mycustomtableendpoint.datacenter-1.ingest.monitor.azure.com -DcrImmutableId dcr-00000000000000000000000000000000
    
    Use a certificate from an Azure KeyVault to obtain the necessary tokens, determine the latest log in your log analytics workspace, gather Teams Device Usage data from the date of the last existing entry in your log analytics workspace and send that data to your log analytics workspace custom table. 

    .EXAMPLE
    Add-CAMLTeamsDeviceUsageData -VaultSecretName secret-CustomAzureMonitorLog -VaultName kv-CustomAzureMonitorLog -ClientId 00000000-0000-0000-0000-000000000000 -TenantId 00000000-0000-0000-0000-000000000000 -WorkspaceId 00000000-0000-0000-0000-000000000000 -LawTable CAMLTeamsDeviceUsage_CL -DceLogsIngestionUri https://mycustomtableendpoint.datacenter-1.ingest.monitor.azure.com -DcrImmutableId dcr-00000000000000000000000000000000

    Use a secret from an Azure KeyVault to obtain the necessary tokens, determine the latest log in your log analytics workspace, gather Teams Device Usage data from the date of the last existing entry in your log analytics workspace and send that data to your log analytics workspace custom table.

    .EXAMPLE
    Add-CAMLTeamsDeviceUsageData -ThumbPrint 0000000000000000000000000000000000000000 -CertStore LocalMachine -ClientId 00000000-0000-0000-0000-000000000000 -TenantId 00000000-0000-0000-0000-000000000000 -WorkspaceId 00000000-0000-0000-0000-000000000000 -LawTable CAMLTeamsDeviceUsage_CL -DceLogsIngestionUri https://mycustomtableendpoint.datacenter-1.ingest.monitor.azure.com -DcrImmutableId dcr-00000000000000000000000000000000

    Use a certificate from the local certificate store to obtain the necessary tokens, determine the latest log in your log analytics workspace, gather Teams Device Usage data from the date of the last existing entry in your log analytics workspace and send that data to your log analytics workspace custom table.

    .LINK
        https://github.com/jklotzsche-msft/CustomAzureMonitorLog
#>
    function Add-CAMLTeamsDeviceUsageData {
    ...
    begin {
        Write-Host "Function 'Add-CAMLTeamsDeviceUsageData' has been triggered"
    ...
    process {
        ...
        # Step 2: Collect Teams Device Usage data from yesterday
        $resultTeamsDeviceUsage = Get-CAMLTeamsDeviceUsage -Token $reportingToken -Period $Period

        # Step 3: Request a token for accessing Azure data collection endpoint
        if($VaultCertificateName) {
            # Using certificate from Azure KeyVault
        ...
        # Step 4: Send log data to your Azure data collection endpoint aka Azure Log Analytics
        foreach($teamsDeviceUsage in $resultTeamsDeviceUsage) {
            Write-Host "Chunk $($resultTeamsDeviceUsage.IndexOf($teamsDeviceUsage) + 1) / $($resultTeamsDeviceUsage.count) - Add-CAMLDataToCustomLogTable"
            Add-CAMLDataToCustomLogTable -Body $teamsDeviceUsage -Token $monitorToken -DceLogsIngestionUri $DceLogsIngestionUri -DcrImmutableId $DcrImmutableId -LawTable $TeamsDeviceUsageLawTable
        }    
    }
    end {
        Write-Host "Function 'Add-CAMLTeamsDeviceUsageData' ended"
    }
}