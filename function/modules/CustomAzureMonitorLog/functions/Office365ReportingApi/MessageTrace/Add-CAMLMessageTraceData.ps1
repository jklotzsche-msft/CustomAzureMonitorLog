<#
	...
	.PARAMETER MessageTraceLawTable
	...
	The string you provide will be used to concatenate the data collection endpoint url, combining the DceLogsIngestionUri, DcrImmutableId and MessageTraceLawTable.
    ...
    .EXAMPLE
    Add-CAMLMessageTraceData -VaultCertificateName cert-CustomAzureMonitorLog -VaultName kv-CustomAzureMonitorLog -ClientId 00000000-0000-0000-0000-000000000000 -TenantId 00000000-0000-0000-0000-000000000000 -WorkspaceId 00000000-0000-0000-0000-000000000000 -MessageTraceLawTable CAMLMessageTrace_CL -DceLogsIngestionUri https://mycustomtableendpoint.datacenter-1.ingest.monitor.azure.com -DcrImmutableId dcr-00000000000000000000000000000000
    ...
    .EXAMPLE
    Add-CAMLMessageTraceData -VaultSecretName secret-CustomAzureMonitorLog -VaultName kv-CustomAzureMonitorLog -ClientId 00000000-0000-0000-0000-000000000000 -TenantId 00000000-0000-0000-0000-000000000000 -WorkspaceId 00000000-0000-0000-0000-000000000000 -MessageTraceLawTable CAMLMessageTrace_CL -DceLogsIngestionUri https://mycustomtableendpoint.datacenter-1.ingest.monitor.azure.com -DcrImmutableId dcr-00000000000000000000000000000000
    ...
    .EXAMPLE
    Add-CAMLMessageTraceData -ThumbPrint '0000000000000000000000000000000000000000' -CertStore 'CurrentUser' -ClientId '00000000-0000-0000-0000-000000000000' -TenantId '00000000-0000-0000-0000-000000000000' -WorkspaceId '00000000-0000-0000-0000-000000000000' -MessageTraceLawTable 'CAMLMessageTrace_CL' -DceLogsIngestionUri 'https://mycustomtableendpoint.datacenter-1.ingest.monitor.azure.com' -DcrImmutableId 'dcr-00000000000000000000000000000000'
...
#>
    function Add-CAMLMessageTraceData {
    ...
    begin {
        Write-Host "Function 'Add-CAMLMessageTraceData' has been triggered"
    ...
    process {
        ...
        $resultStartDate = Get-CAMLCustomLogTable -Token $logAnalyticsToken -WorkspaceId $WorkspaceId -LawTable $MessageTraceLawTable -ResultSize 1 -SortProperty 'Received' -ReturnProperty 'EndDate'
        ...
        # Step 4: Collect message tracking log data from last run time until now
        ...
        # Step 6: Send message tracking log data to your Azure data collection endpoint aka Azure Log Analytics Workspace
        foreach($messageTrace in $resultMessageTrace) {
            Write-Host "Chunk $($resultMessageTrace.IndexOf($messageTrace) + 1) / $($resultMessageTrace.count) - Add-CAMLDataToCustomLogTable"
            Add-CAMLDataToCustomLogTable -Body $messageTrace -Token $monitorToken -DceLogsIngestionUri $DceLogsIngestionUri -DcrImmutableId $DcrImmutableId -LawTable $MessageTraceLawTable
        }
    }
    end {
        Write-Host "Function 'Add-CAMLMessageTraceData' ended"
    }
}